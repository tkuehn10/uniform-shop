...
Following a request to keep the number of cloud services as small as possible, item photos are no longer stored in Supabase Storage. Instead, each photo is compressed and resized client-side (roughly 400px on the long edge, stepping down to 320/240/160px if needed, capped around 50KB) and stored as a `bytea` blob directly in Postgres, in a dedicated `item_photos` table (see `database-schema.md`). This removes Supabase Storage from the architecture entirely — the system now uses exactly one backend service (Supabase's Postgres database + Auth) instead of two.

**Trade-off accepted:** the database grows somewhat larger and backups get a bit heavier as photos are added (bounded in practice — a catalog of a few hundred items at ~50KB each is only a few tens of MB total, well inside the free tier). If the shop ever wants sharper or larger photos than this cap comfortably allows, this should be revisited and moved to a real file-storage service instead.
...
- **Supabase — still the right fit, arguably more so now.** With Storage no longer needed, the backend is just two tightly-coupled parts of one project (Postgres + Auth) rather than three separate services. The photo size math holds up comfortably: even a few hundred items at ~50KB each is only tens of MB, nowhere near the free tier's 500MB database cap.
